...
echo " - Copiando $dbfile al contenedor como archivo temporal..."
docker cp "$dbfile" $dockerContainer:/
sleep 1;

echo " - Descomprimiendo archivo temporal "$filename".gz..."
docker exec $dockerContainer gunzip -kf "$filename".gz
sleep 1;

echo " - Removiendo archivo temporal $filename.gz..."
...
echo " - Removiendo archivo temporal $filename..."
